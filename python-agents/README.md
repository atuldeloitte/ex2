...
- `mcp_server/server.py` — MCP server (streamable-http, port 8000). Wraps
  ...
  `adjust_stock`, `delete_stock_item`, `list_low_stock`). Every tool takes
  `customer_id` so one server instance serves every customer without mixing
  up their data.
...
- `agents/memory.py` — a `SqliteSaver` checkpointer. Conversations are keyed
  by `thread_id = customer_id`, so each customer's conversation history and
  agent state persists across turns and across restarts.
...
Try asking things like "what's low on stock for acme?" or "add 10 units of
WIDGET-001" — those get routed to the inventory agent, which calls the MCP
tools scoped to that customer. Ask something unrelated and it's routed to
the general agent instead. Run the CLI again later with the same
`--customer-id` and the conversation history is still there, loaded from
`agent_memory.db`.
